...
def _qdropconnect(W, mode):
    shapel = W
    W_shape = np.array(shapel)
    fan_in, fan_out = shapel[-2], shapel[-1]
    # print(W_shape)
    QM = np.ones([fan_in, fan_out], np.float32)
    # drop_num = int(fan_out / 6)
    #drop_num = np.random.random_integers(0,8)
    drop_num = 4
    if drop_num > fan_in or drop_num == 0 or mode is not 'train':
        # print('No')
        return QM
    # print(fan_in,fan_out,drop_num)
    drop_index = np.random.random_integers(fan_out - 1, size=drop_num)
    mask = np.eye(fan_in, fan_in)
    for i, n in enumerate(drop_index):
        QM[:, n] = mask[:, i]
...
def inference(images, conv1_channels, conv2_channels, fc1_units, fc2_units, mode):
    """Build the IndianPines model up to where it may be used for inference.
    Args:
    images: Images placeholder, from inputs().
    ...
    with tf.variable_scope('h_conv2') as scope:
        ...
        #if mode == 'train':
        	#weights = qdropconnect(weights)
        inp = [KERNEL_SIZE, KERNEL_SIZE, conv1_channels, conv2_channels]
        QM = tf.py_func(_qdropconnect, [inp, mode], tf.float32)
        weights = tf.multiply(weights, QM)
